...
import { useParams, useNavigate, usePathname } from "@/lib/navigation";
...
const CurrentAffairDetail = () => {
  ...
  const maybeRedirectToCanonical = (item) => {
    if (!item?.title || redirectingRef.current) return;
    const canonicalSlug = slugify(item.title);
    if (!canonicalSlug) return;
    const currentSlug = slug ? decodeURIComponent(String(slug)) : "";
    if (currentSlug === canonicalSlug) return;
    const canonicalPath = `/current-affairs/${canonicalSlug}`;
      ...
      try {
        ...
        if (!id && slug) {
          ...
          if (single) {
            setPost(single);
            setAffairs([single]);
            maybeRedirectToCanonical(single);
            return;
          }
          ...
          if (match) {
            setPost(match);
            maybeRedirectToCanonical(match);
            return;
          }
  ...
  <div className="relative min-h-screen w-full py-8 px-4 bg-gradient-to-br from-sky-50 via-white to-indigo-50" style={{ marginBottom: '-64px' }}>
    <SEOHead pathname={seoPathname} customData={seoData} />
