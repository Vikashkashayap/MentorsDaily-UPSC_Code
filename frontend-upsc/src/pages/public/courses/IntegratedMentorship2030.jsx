...
import Imp2030View from "./Imp2030View";
import "./imp2030LandingExtras.css";
import {
  getDefaultImp2030DetailPage,
  mergeImpDetailPage,
} from "./imp2030DetailDefaults";
...
function Imp2030DetailSkeleton() {
  return (
    <div className="bg-white overflow-x-hidden pb-10 animate-pulse">
      <div className="h-10 bg-[#042F2E]" />

      <section className="bg-gradient-to-br from-[#042F2E] via-[#115E59] to-[#134E4A] pt-16 pb-14">
...
export default function IntegratedMentorship2030({ courseSlug = IMP_2030_SLUG, hideWeeklyPlan = true }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const base = getDefaultImp2030DetailPage();
        if (c) {
          setCourse(c);
  ...
  const resolvedCourseId =
    course?._id ||
    (typeof import.meta.env.VITE_IMP_2030_COURSE_ID === "string"
      ? import.meta.env.VITE_IMP_2030_COURSE_ID.trim()
      : "");
  ...
  /** When the API does not return the course (common on local dev), still show the payment UI using defaults + detail; set VITE_IMP_2030_COURSE_ID for a real Razorpay order. */
  const paymentCourseBase =
    ...
    (() => {
      ...
      return {
        _id: id || undefined,
        title: detail?.hero?.cardTitle ?? "Integrated Mentorship Program - 2030",
        slug: slugToFetch,
        basePrice,
        sellingPrice,
        discountPercentage,
      ...
      {showPaymentForm && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl relative overflow-hidden">
            <div className="bg-gradient-to-r from-[#115E59] to-[#0F766E] p-4 text-white">
