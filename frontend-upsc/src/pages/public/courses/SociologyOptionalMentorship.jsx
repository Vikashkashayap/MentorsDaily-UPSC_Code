...
import "./sociologyOptionalExtras.css";

export const SOCIOLOGY_OPTIONAL_SLUG = "sociology-optional-course";
...
const Chevron = () => (
  <svg
    className="soc-chev w-5 h-5 text-gray-400 shrink-0 transition-transform"
    fill="none"
    stroke="currentColor"
    viewBox="0 0 24 24"
  >
...
const PAIN_POINTS = [
  ["01", '"I know the thinkers but can\'t connect them."', "Marx, Weber, Durkheim sit in separate boxes. The exam wants you to make them talk to each other."],
  ["02", '"My answers describe. They don\'t analyse."', "You reproduce theory, but Paper II wants you to apply it to caste, agrarian change, kinship — with judgement."],
  ["03", '"I can\'t bring in Indian examples."', "Western theory in Paper I, Indian reality in Paper II — and no bridge between the two on the page."],
...
const COMPARISON_ROWS = [
  ...
  ["To link thinkers & apply theory", "Topic-by-topic lectures in isolation", "SML linkage trained on every single topic"],
  ["Answers that are read, not scored", "A number, weeks later", "SAE competency feedback + rewrite loop"],
  ['To never fall behind', 'Linear ladder — miss a rung, panic', "The Revolution — every topic comes back"],
  ['To not feel out of place', '"Advanced batch" vs "beginner batch"', "One room, SML depth adapts to you"],
];

const FRAMEWORKS = [
  ["DDF · Demand Decode", "Read what the question actually asks — directive, dimension, and the thinker it's fishing for."],
  ["TLF · Thinker Linking", "Put any two thinkers in conversation — agree, contrast, extend — on demand."],
  ["IAB · Indian Application Bridge", "Carry a Western concept across to caste, agrarian change, kinship, movements — with a real example."],
  ["CPB · Cross-Paper Bridge", "Feed Paper I theory into Paper II answers, and vice-versa — one mind, both papers."],
  ["DDL · Definition & Diagram", "Open with a crisp definition and a diagram the examiner remembers."],
  ["JES · Judgement & Evaluation", "End with a defended position — the sociological judgement most answers never reach."],
];

const CURRICULUM = [
  ["01", "Foundations & Methods", "Sociology as discipline & science · research methods · the sociological approach."],
  ["02", "Classical & Modern Thinkers", "Marx · Weber · Durkheim · Parsons · Merton · Mead — with TLF linkage between them."],
  ["03", "Stratification, Work & Politics", "Class, caste, gender · power & bureaucracy · work & economic life."],
  ["04", "Family, Religion & Change", "Family & kinship · religion & secularisation · social change & modernity."],
  ["05", "Paper II · Indian Society", "Indian sociology · village & agrarian structure · caste · tribes · industrialisation — via IAB."],
  ["06", "Paper II · Contemporary India", "Politics & social change · population · movements · visions of social change."],
];

const DELIVERABLES = [
  ["Daily notes (Hindi/Eng)", "One concept, fully developed, PYQ-tagged, diagram-backed."],
  ["Framework Handbook", "All six SML engines, defined and ready to apply."],
  ["Thinker Linkage Maps", "Visual TLF maps connecting the classical & modern thinkers."],
  ["Question Bank", "Decoded PYQs and practice questions by demand type."],
  ["Model Answer Library", "Reference answers annotated by SML layer and framework."],
  ["Revision Sheets", "One-pager per topic, reused across the Revolution."],
  ["SAE Evaluation Sheets", "The rubric for scoring your own and peers' answers."],
...
const WHO = [
  ...
  ["Switching to Sociology", "The Foundation track builds your base from zero while the Orbit keeps you in the cohort."],
  ["Non-humanities background", 'Afraid it\'s "too theoretical"? SML makes theory concrete — a concept, a link, an example.'],
  ["Hindi-medium aspirant", "Every note is bilingual; write and get evaluated in the medium you'll use in the exam."],
];

const OUTCOMES = [
  "Decode the exact demand of any Sociology question.",
  "Put any two thinkers in conversation on demand.",
  "Bridge Western theory to Indian society with real examples.",
  "Move Paper I theory into Paper II answers, and back.",
  "Open with a definition and a diagram the examiner remembers.",
  "End every answer with a defended sociological judgement.",
  "Write a full answer within the time limit.",
...
const FAQ_GROUPS = [
  ...
  },
  {
    heading: "The method",
    items: [
      ["What is SML?", "The 3-Layer Sociological Mind: See the concept, Link it (to thinkers, the other paper, Indian society), Argue it under time. Every topic and every answer runs through all three layers, in order."],
      ["I can never connect thinkers. Does this help?", "That's exactly what the Thinker Linking Framework (TLF) trains — putting any two thinkers in conversation on demand, instead of memorising them in separate boxes."],
      ["My Paper II answers lack Indian examples.", "The Indian Application Bridge (IAB) trains you to carry a Western concept across to caste, agrarian change, kinship or movements with a concrete example — every time."],
      ...
      ["Does it cover both papers fully?", "Yes — Paper I and Paper II, complete, and deliberately cross-linked (CPB) so the two papers reinforce each other instead of being learned twice."],
    ],
  },
  {
    heading: "Evaluation & feedback",
    items: [
      ["How are my answers evaluated?", "On the SAE Rubric (Sociology Answer Evaluation) — a breakdown across demand, thinker linkage, Indian application, judgement, presentation and time, with a specific next fix. Not one vague number."],
      ...
      ["Can I track my progress?", "Your dashboard charts SAE competency growth month over month and flags weak areas across the syllabus."],
    ],
  },
  {
...
const ORBIT_LOOP = [
  ...
  ["2", "Assess", "Score on the SAE rubric."],
  ["3", "Reflect", "Name the missing link or judgement."],
  ["4", "Rewrite", "Produce the improved answer."],
];

export default function SociologyOptionalMentorship() {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll(".soc-reveal").forEach((el) => obs.observe(el));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const envelope = await getCourseBySlug(SOCIOLOGY_OPTIONAL_SLUG);
  ...
  const paymentCourse = {
    ...
    title: course?.title || "Sociology Optional Mentorship",
    slug: SOCIOLOGY_OPTIONAL_SLUG,
  ...
  return (
    <div className="soc-root bg-white text-gray-900 antialiased">
      <SEO
        title={course?.metaTitle || course?.title || "Sociology Optional Mentorship"}
        description={
          course?.metaDescription ||
          "An always-running, join-anytime Sociology Optional mentorship built on the 3-Layer Sociological Mind (SML): See, Link, Argue."
        }
        image={getCourseOgImageUrl(course)}
        url={`/${SOCIOLOGY_OPTIONAL_SLUG}`}
      />
      <Helmet>
        <meta
          name="keywords"
          content="UPSC Sociology Optional, sociology mentorship, sociology optional 2026, sociology answer writing, evergreen sociology course, MentorsDaily"
        />
        <script type="application/ld+json">
          {JSON.stringify({
            "@context": "https://schema.org",
            "@type": "Course",
            name: "MentorsDaily Sociology Optional Mentorship",
            description:
              "An always-running, join-anytime Sociology Optional mentorship built on the 3-Layer Sociological Mind (SML).",
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-8 pt-16 pb-14 grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7">
            <div className="flex items-center gap-2 flex-wrap mb-5">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white border border-brand-100 text-brand-700 text-xs font-bold soc-badge uppercase">
                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M10 1l2.39 5.84L18.5 7.5l-4.75 4.05L15.2 18 10 14.85 4.8 18l1.45-6.45L1.5 7.5l6.11-.66L10 1z" />
                </svg>
                Flagship Optional
              </span>
              <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-brand-600 text-white text-xs font-bold soc-badge uppercase">
                Always running · Join any day
              </span>
            </div>
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">UPSC Sociology Optional Mentorship</p>
            <h1 className="mt-3 text-4xl md:text-6xl font-black tracking-tight leading-[1.05]">
              Stop collecting notes.
              <br />
              Start <span className="text-brand-600">seeing</span> like a sociologist.
            </h1>
            <p className="mt-6 text-lg text-gray-600 max-w-2xl">
              Sociology is not a reading subject — it is a seeing subject. Most aspirants own three sets of notes and still
              write description, not analysis. This program teaches Sociology as a system:{" "}
              <strong>See the concept, Link it to Indian society, Argue it under exam time.</strong> And because it runs
              every single day, you can <strong>join today</strong> — you&apos;ll never have to &quot;catch up&quot;.
            </p>
            <ul className="mt-7 grid sm:grid-cols-2 gap-2.5 max-w-2xl">
              {[
                "Join on any date — no batch, no start line to miss",
                "Link any two thinkers on demand — stop memorising in silos",
                "Bridge Western theory to Indian society, every answer",
          ...
          <div className="lg:col-span-5">
            <div className="rounded-2xl border-2 border-brand-600 bg-white shadow-xl shadow-brand-900/10 overflow-hidden">
              <div className="bg-brand-600 text-white px-6 py-3 flex items-center justify-between">
                <span className="font-bold">Sociology Optional Mentorship</span>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-white/15 soc-badge uppercase">
                  Full Access
                </span>
              </div>
              <div className="p-6">
                <p className="text-xs font-bold soc-badge uppercase text-brand-600">Membership</p>
      ...
      {/* ============ 2. CORE BELIEF ============ */}
      <section className="bg-brand-900 text-white soc-grain">
        <div className="max-w-5xl mx-auto px-4 md:px-8 py-16 text-center">
          <p className="text-xs font-bold soc-badge uppercase text-brand-200">Our core belief</p>
          <p className="mt-4 text-2xl md:text-4xl font-bold leading-tight">
            &quot;Sociology is not a reading subject. It is a{" "}
            <span className="text-brand-200">seeing and connecting</span> subject — the marks come from linkage, not
            information.&quot;
          </p>
          <p className="mt-6 text-brand-200/80 text-sm max-w-2xl mx-auto">
            Everyone reads the same Haralambos, the same Ritzer. The topper isn&apos;t the one who read more — it&apos;s the
            one who can link a thinker to a headline and defend a position. This program trains that.
          </p>
        </div>
      </section>

      {/* ============ 3. PAIN POINTS ============ */}
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-20 soc-reveal">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Sound familiar?</p>
      ...
      <section className="bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-20 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">The category problem</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
              Why the usual Sociology course leaves you exactly where you started
            </h2>
            <p className="mt-4 text-lg text-gray-600">
      ...
      {/* ============ 5. SML SYSTEM ============ */}
      <section id="system" className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">The system</p>
          <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">The 3-Layer Sociological Mind</h2>
          <p className="mt-4 text-lg text-gray-600">
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-5">
          {[
            ...
            ["2", "Link", "Connect the concept to other thinkers, to the other paper, and to Indian society — the linkage that separates an 11 from a 6."],
        ...
        <div className="mt-14 max-w-3xl">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">The engines inside SML</p>
        ...
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FRAMEWORKS.map(([tag, desc]) => (
            <div key={tag} className="rounded-2xl border border-gray-100 p-6">
              <div className="text-xs font-bold text-brand-700 soc-badge uppercase">{tag}</div>
              <p className="mt-2 text-sm text-gray-600">{desc}</p>
            </div>
          ))}
        </div>
      ...
      <section id="running" className="bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">How it runs</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
          ...
          <div className="mt-12 grid lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-2xl bg-white border border-gray-200 p-7">
              ...
              <p className="mt-3 text-sm text-gray-600">
                A self-paced track in fixed order — discipline &amp; methods → thinkers → stratification → Indian society.
              </p>
              <ul className="mt-4 space-y-2 text-sm text-gray-700">
          ...
          <div className="mt-6 rounded-2xl bg-brand-900 text-white soc-grain p-7 md:p-10">
            <p className="text-xs font-bold soc-badge uppercase text-brand-200">The Revolution — why nobody is ever &quot;late&quot;</p>
            <div className="mt-4 grid md:grid-cols-4 gap-4">
              {[
                ["1 loop = full syllabus", "Paper I + Paper II is one revolution. It ends, then restarts."],
                ["Miss a topic? It returns", "Skipped caste in March? It comes back next revolution."],
      ...
      {/* ============ 7. INSIDE ONE DAY ============ */}
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Inside one day</p>
        ...
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ["Step 1 · ~30 min", "See the note", "One concept, fully developed — definition, thinker, PYQ angle, Indian example."],
            ["Step 2 · ~15 min", "Do the Link task", "Connect the concept to a thinker or apply it to Indian society."],
            ...
            >
              <div className={`text-xs font-bold soc-badge uppercase ${highlight ? "text-brand-100" : "text-brand-700"}`}>
                {step}
              </div>
      ...
      <section id="curriculum" className="bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Curriculum</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
      ...
      {/* ============ 9. EVALUATION ============ */}
      <section id="evaluation" className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Evaluation</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
              You&apos;ll never get &quot;6/10, add more content&quot; again
            </h2>
            <p className="mt-4 text-lg text-gray-600">
              Every answer is scored on the <strong>SAE Rubric — Sociology Answer Evaluation</strong>.
            </p>
          </div>
          <div className="rounded-2xl border border-gray-200 shadow-xl shadow-brand-900/5 p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-xs font-bold text-gray-400 soc-badge uppercase">Sample SAE Scorecard</div>
                <div className="font-bold text-lg">Answer · Paper II · Caste</div>
              </div>
            ...
            <div className="mt-5 space-y-3">
              {[
                ...
                ["Thinker linkage", 60, "accent"],
                ["Indian application", 70, "brand"],
                ["Sociological judgement", 60, "accent"],
            ...
            <div className="mt-5 rounded-lg bg-brand-50 p-4 text-sm text-brand-900">
              <span className="font-bold">Fix next:</span> Thinker linkage. Put Ambedkar and Srinivas in conversation for
              one line of real judgement.
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">The signature loop</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
          ...
          <div className="mt-12 flex flex-col md:flex-row items-stretch gap-3">
            {ORBIT_LOOP.map(([num, title, desc]) => (
              <React.Fragment key={num}>
                ...
                <div className="hidden md:grid place-items-center text-2xl soc-flow-arrow">→</div>
              </React.Fragment>
            ))}
      ...
      <section className="bg-brand-900 text-white soc-grain">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-200 soc-badge uppercase">What you receive</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
              A complete Sociology toolkit — not just classes
            </h2>
          </div>
      ...
      <section id="who" className="bg-gray-50 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Who it&apos;s for</p>
            <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
      ...
      <section className="bg-brand-900 text-white soc-grain">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
          <div className="max-w-3xl">
            <p className="text-sm font-semibold text-brand-200 soc-badge uppercase">Outcomes</p>
      ...
      {/* ============ 14. ENROLL ============ */}
      <section id="enroll" className="max-w-7xl mx-auto px-4 md:px-8 py-24 soc-reveal">
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">Join</p>
          <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">
        ...
        <div className="mt-12 max-w-xl mx-auto rounded-3xl border-2 border-brand-600 shadow-2xl shadow-brand-900/10 overflow-hidden">
          <div className="bg-brand-600 text-white px-8 py-4 flex items-center justify-between">
            <span className="font-bold">Sociology Optional Mentorship</span>
            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-white/15 soc-badge uppercase">Full Access</span>
          </div>
          <div className="p-8">
      ...
      {/* ============ 15. FAQ ============ */}
      <section id="faq" className="max-w-4xl mx-auto px-4 md:px-8 py-24 soc-reveal">
        <div className="text-center max-w-2xl mx-auto">
          <p className="text-sm font-semibold text-brand-700 soc-badge uppercase">FAQ</p>
          <h2 className="mt-2 text-3xl md:text-4xl font-extrabold tracking-tight">Every question, answered</h2>
        </div>
        <div className="mt-10 space-y-8">
          {FAQ_GROUPS.map((group) => (
            <div key={group.heading}>
              <h3 className="text-sm font-bold text-gray-400 soc-badge uppercase mb-3">{group.heading}</h3>
      ...
      {/* ============ 16. FINAL CTA ============ */}
      <section className="bg-brand-900 text-white soc-grain">
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-24 text-center soc-reveal">
          <h2 className="text-3xl md:text-5xl font-black tracking-tight leading-tight">
            Sociology can be learned as a system —
            <br />
            and you can start today.
          </h2>
          <p className="mt-6 text-lg text-brand-100/80 max-w-2xl mx-auto">
            If you&apos;ve been &quot;starting Sociology next month&quot; for six months, the missing piece was never more
