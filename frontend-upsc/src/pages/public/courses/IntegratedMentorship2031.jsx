...
import Imp2031View from "./Imp2031View";
import "./imp2031LandingExtras.css";
import {
  IMP_2031_SLUG,
  getDefaultImp2031DetailPage,
  mergeImpDetailPage,
} from "./imp2031DetailDefaults";
...
export default function IntegratedMentorship2031() {
  const [course, setCourse] = useState(null);
  const [detail, setDetail] = useState(() => getDefaultImp2031DetailPage());
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const envelope = await getCourseBySlug(IMP_2031_SLUG);
        const c = unwrapCourseFromSlugResponse(envelope);
        if (cancelled) return;
        const base = getDefaultImp2031DetailPage();
        if (c) {
          setCourse(c);
        ...
        console.error(e);
        if (!cancelled) setDetail(getDefaultImp2031DetailPage());
      } finally {
  ...
  const heroSelling = dailyPlan?.price != null ? Number(dailyPlan.price) : Number(course?.sellingPrice ?? course?.basePrice ?? 190000);
  ...
  const resolvedCourseId =
    course?._id ||
    (typeof import.meta.env.VITE_IMP_2031_COURSE_ID === "string"
      ? import.meta.env.VITE_IMP_2031_COURSE_ID.trim()
      : "");
  ...
  const paymentCourseBase =
    ...
    (() => {
      ...
      return {
        _id: id || undefined,
        title: detail?.hero?.cardTitle ?? "Integrated Mentorship Program - 2031",
        slug: IMP_2031_SLUG,
        basePrice: heroBase,
  ...
  if (loading) return <Imp2031DetailSkeleton />;
  ...
  return (
    <>
      <SEO
        ...
        url="/integrated-mentorship-2031"
      />
      <Helmet>
