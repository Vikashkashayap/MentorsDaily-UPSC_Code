...
function renderHeroTitleLine(line, i) {
  ...
  return (
    <span key={i} className="block">
      {accented ? <span className="text-[#FDBA74]">{line.text}</span> : line.text}
    </span>
  );
}

export default function Imp2029View({
  d,
  sellingPrice,
  basePrice,
  discountPercentage,
  ...
  const dashboardFallback = [
    {
      ...
      body: "Visual journey of your entire 48-month syllabus broken into structured, week-by-week modules.",
      icon: "Map",
  ...
  return (
    <div className="imp2027-root bg-white overflow-x-hidden pb-24 md:pb-0">
      {/* Announcement */}
      <div className="bg-[#9A3412] text-[#FFEDD5] text-center py-2 px-6 text-sm font-medium">
        <strong className="text-[#FDBA74]">{d.announcement.strong}</strong> {d.announcement.text}{" "}
        <button
          type="button"
          className="text-[#FDBA74] font-bold ml-3 bg-transparent border-0 cursor-pointer p-0 font-inherit underline-offset-2 hover:underline"
      ...
      <section className="relative bg-gradient-to-br from-[#9A3412] via-[#C2410C] to-[#EA580C] pt-16 pb-14 overflow-hidden" id="top">
        <div className="pointer-events-none absolute -top-[120px] -right-[120px] w-[500px] h-[500px] rounded-full bg-[radial-gradient(circle,rgba(251,146,60,0.25)_0%,transparent_70%)]" />
        <div className="max-w-[1180px] mx-auto px-6 grid lg:grid-cols-[1fr_420px] gap-14 lg:gap-16 items-center">
          <div>
            <div className="imp-fade-up inline-flex items-center gap-1.5 bg-[rgba(251,146,60,0.2)] border border-[rgba(251,146,60,0.45)] text-[#FDBA74] text-[0.82rem] font-bold px-3.5 py-1.5 rounded-full mb-5">
            ...
            <div className="imp-fade-up delay-2 flex flex-wrap gap-2.5 mb-8">
              {d.hero.perks.map((p, i) => (
                ...
                >
                  <DynIcon name="CheckCircle" className="text-[#FDBA74] w-3.5 h-3.5" />
                  {p}
                </div>
              ))}
            </div>
          ...
          <div className="imp-fade-up delay-2 bg-white rounded-[20px] shadow-[0_24px_80px_rgba(0,0,0,0.28)] overflow-hidden" id="enroll">
            <div className="bg-gradient-to-br from-[#C2410C] to-[#EA580C] px-6 py-6 text-center">
            ...
            <div className="p-6">
              ...
              <p className="flex items-center justify-center gap-1.5 text-xs text-[#6B7280] mt-3.5">
                <DynIcon name="Shield" className="text-[#E86B2A]" size={14} /> {d.hero.cardTrust}
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Trust bar */}
      <div className="bg-[#C2410C] py-5">
        <div className="max-w-[1180px] mx-auto px-6 flex flex-wrap justify-around gap-5 items-center">
          {d.trustBar.map((t, i) => (
            <div key={i} className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-[10px] bg-white/10 flex items-center justify-center text-[#FDBA74]">
      ...
      {/* Pricing */}
      <section className="bg-[#9A3412] py-20" id="pricing">
        <div className="max-w-[1180px] mx-auto px-6">
          <div className="text-center mb-3">
            <span className="inline-block bg-[#FFEDD5] text-[#C2410C] text-xs font-bold uppercase tracking-wider px-3.5 py-1 rounded-full mb-3.5">
          ...
          {showWeeklyPlan ? (
            <p className="text-center text-sm text-white/45 mb-12">
              Scroll down for a <strong className="text-[#FDBA74]">feature-by-feature comparison</strong> table ↓
            </p>
          ) : (
            <div className="mb-12" />
          )}

          <div className={`grid ${showWeeklyPlan ? "md:grid-cols-2" : "grid-cols-1"} gap-7 mb-12 items-stretch`}>
            {/* Daily first (left) */}
